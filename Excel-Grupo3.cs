    ...
    public partial class Form1 : Form
    {
        ...
        private void CBXOperaciones_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // *** LÍNEA AGREGADA: Lista para almacenar celdas con contenido no numérico ***
                var celdasConTexto = new List<DataGridViewCell>();

                foreach (var celda in seleccionadas)
                {
                    string texto = celda.Value?.ToString().Replace("L.", "").Replace("$", "").Trim() ?? "";

                    // *** LÍNEAS MODIFICADAS: Se cambió la lógica para detectar celdas vacías vs celdas con contenido no numérico ***
                    if (string.IsNullOrEmpty(texto))
                    {
                        // *** LÍNEA AGREGADA: Celda vacía ***
                        celdasVacias.Add(celda);
                    }
                    else if (double.TryParse(texto, NumberStyles.Any, CultureInfo.InvariantCulture, out double numero) ||
                    ...
                    {
                        // *** LÍNEA AGREGADA: Celda con contenido no numérico (letras, símbolos, etc.) ***
                        celdasConTexto.Add(celda);
                    }
                }

                // *** LÍNEAS AGREGADAS: Validación específica para celdas con contenido no numérico ***
                if (celdasConTexto.Count > 0)
                {
                    string celdasInvalidas = string.Join(", ", celdasConTexto.Select(c =>
                        $"{GetColumnName(c.ColumnIndex)}{c.RowIndex + 1} ('{c.Value}')"));

                    MessageBox.Show($"Error: Las siguientes celdas contienen texto o símbolos que no son números válidos:\n\n{celdasInvalidas}\n\nPara realizar operaciones matemáticas, todas las celdas deben contener únicamente números.",
                                    "Contenido no numérico detectado",
        ...
        private string GetColumnName(int columnIndex)
        {
            ...
            // Las columnas A-Z empiezan en el índice 1
